...
func (i *IfStmt) Generate() error {
	...
	if !expr.Type.IsBool() {
		return utils.WithPos(fmt.Errorf("cannot use %s as condition", expr.Type.String()), i.Scope.Current().File, i.Pos)
	}
...
func (w *WhileStmt) Generate() error {
	...
	if !expr.Type.IsBool() {
		return utils.WithPos(fmt.Errorf("cannot use %s as condition", expr.Type.String()), w.Scope.Current().File, w.Pos)
	}
